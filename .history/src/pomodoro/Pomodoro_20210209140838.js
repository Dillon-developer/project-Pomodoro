...
function Pomodoro() {
  ...
  const decreaseFocusDuration = () => {
    if (focusDuration > 5 && !isTimerRunning && onPlay) 
      setFocusDuration(minutes => minutes -= 5);
    
  }

  const increaseFocusDuration = () => {
    if (focusDuration < 60 && !isTimerRunning && onPlay) 
      setFocusDuration(minutes => minutes += 5);
    
  }

  const decreaseBreakDuration = () => {
    if (breakDuration > 1 && !isTimerRunning && onPlay) 
      setBreakDuration(minutes => minutes -= 1);
    
  }

  const increaseBreakDuration = () => {
    if (breakDuration < 15 && !isTimerRunning && onPlay) 
      setBreakDuration(minutes => minutes += 1);
